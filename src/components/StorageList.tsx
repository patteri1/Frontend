    ...
    const setAmountToStorageHandler = async (
        ...
        try {
            const { data } = await setAmountToStorage({
                variables: { locationId, productId, palletAmount },
            })
            console.log(
                'Updated palletAmount for product:',
                data.setAmountToStorage
            )
        } catch (error) {
            console.error('Error setting palletAmount for product', error)
        }
    }
                    ...
                    const storageItems = location.storages.map((storage) => {
                        return {
                            title: storage.product
                                ...
                                : 'Unknown Product',
                            content: storage.palletAmount
                                ? `${storage.palletAmount}`
                                : 'Unknown Amount',
            ...
            {loading ? (
                ...
                storageCardsData.map((data, index) => (
                    <StorageCard
                        ...
                        onStorageUpdate={(productId, palletAmount) =>
